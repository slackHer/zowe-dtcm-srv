...
export class TempTestProfiles {
    ...
    public static async createProfiles(testEnvironment: ITestEnvironment, profileTypes: string[] = []) {
        ...
        for (const type of profileTypes) {
            if (type === "myProfileType") {
                profileNames.myProfileType.push(await TempTestProfiles.createMyProfileTypeProfile(testEnvironment));
            } else if (type === "zosmf") {
    ...
    /**
     * Helper to create a CICS profile from test properties
     ...
     */
    private static async createMyProfileTypeProfile(testEnvironment: ITestEnvironment) {
        // TODO : update and rename this function for use with your profile type, if any
        const profileName: string = "tmp_myProfileType" + uuidv4().substring(0, TempTestProfiles.MAX_UUID_LENGTH);
        const testProperties = testEnvironment.systemTestProperties.myPropertiesCategory; // TODO: find relevant test properties here

        // TODO: update this to match the syntax of your profile creation command
        const createProfileScript = TemporaryScripts.SHEBANG +
            `${TemporaryScripts.ZOWE_BIN} profiles create myProfileType ${profileName} --user ${testProperties.user} -p ` +
            `${testProperties.password} ` +
            ` --host ${testProperties.host} --port ${testProperties.port}`;
        ...
        if (output.status !== 0 || output.stderr.toString().trim().length > 0) {
            throw new ImperativeError({
                msg: "Creation of myProfileType profile '" + profileName + "' failed! You should delete the script: \n'" + scriptPath + "' " +
        ...
        this.log(testEnvironment, `Created myProfileType profile '${profileName}'. Stdout from creation:\n${output.stdout.toString()}`);
        return profileName;
    }
