...
class IO {
    ...
    static normalizeExtension(extension) {
        ...
        if (!util_1.isNullOrUndefined(extension) && extension.length > 0 && extension[0] !== ".") {
     ...
     * @memberof IO
     */
    static processNewlines(original) {
        expect_1.ImperativeExpect.toBeDefinedAndNonBlank(original, "original");
            ...
            try {
                fs.writeFile(file, content, IO.UTF8, (err) => {
                    if (!util_1.isNullOrUndefined(err)) {
                        throw new error_1.ImperativeError({ msg: err.message });
                    }
                    resolve();
                });
            }
            catch (error) {
                throw new error_1.ImperativeError({ msg: error.message });
            }
        });
    }
    /**
