...
const CommandProfileLoader_1 = require("./profiles/CommandProfileLoader");
...
class CommandProcessor {
    ...
    constructor(params) {
        ...
        expect_1.ImperativeExpect.toNotBeNullOrUndefined(this.mProfileManagerFactory, `${CommandProcessor.ERROR_TAG} No profile manager factory supplied.`);
    ...
    invoke(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.log.trace(`Invoke parameters:\n${util_1.inspect(params, { depth: null })}`);
            this.log.trace(`Command definition:\n${util_1.inspect(this.definition, { depth: null })}`);
            ...
            try {
                ...
                prepared = yield this.prepare(prepareResponse, params.arguments);
            }
            catch (prepareErr) {
                    ...
                    if (this.definition.positionals != null && this.definition.positionals.length > 0) {
                        for (const positional of this.definition.positionals) {
                            ...
                            if (prepared.args[positionalName] != null) {
                                // string processing
                                if ((util_1.isString(prepared.args[positionalName]) &&
                                    ...
                                    prepared.args[positionalName] =
                                        CliUtils_1.CliUtils.promptForInput(`"${positionalName}" Description: ` +
                                ...
                                // array processing
                                    ...
                                    if ((prepared.args[positionalName] != null &&
                                        ...
                                        util_1.isString(prepared.args[positionalName][0]) &&
                                        (prepared.args[positionalName][0].toUpperCase() === this.promptPhrase.toUpperCase()))) {
                                        // prompt has been requested for a positional
                                        this.log.debug("Prompting for positional %s which was requested by passing the value %s", prepared.args[positionalName][0], this.promptPhrase);
                    ...
                    if (this.definition.options != null && this.definition.options.length > 0) {
                        for (const option of this.definition.options) {
                            ...
                            if (prepared.args[option.name] != null) {
                                // string processing
                                if ((util_1.isString(prepared.args[option.name]) &&
                                    ...
                                    prepared.args[option.name] =
                                        CliUtils_1.CliUtils.promptForInput(`"${option.name}" Description: ${option.description}\nPlease enter "${option.name}":`);
                                ...
                                // array processing
                                    ...
                                    if (((Array.isArray(prepared.args[option.name])) &&
                                        ...
                                        util_1.isString(prepared.args[option.name][0]) &&
            ...
            let validator;
            try {
                validator = yield this.validate(prepared.args, response);
            }
            catch (e) {
                ...
                for (let chainedHandlerIndex = 0; chainedHandlerIndex < this.definition.chainedHandlers.length; chainedHandlerIndex++) {
                    ...
                    try {
                        yield handler.process({
                            ...
                            profiles: prepared.profiles,
                            arguments: ChainedHandlerUtils_1.ChainedHandlerService.getArguments(this.mCommandRootName, this.definition.chainedHandlers, chainedHandlerIndex, chainedResponses, prepared.args, this.log),
                            definition: this.definition,
                            fullDefinition: this.fullDefinition,
                            isChained: true
                        });
    ...
    prepare(response, commandArguments) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.log.trace(`Loading profiles for "${this.definition.name}" command. ` +
                `Profile definitions: ${util_1.inspect(this.definition.profile, { depth: null })}`);
            const profiles = yield CommandProfileLoader_1.CommandProfileLoader.loader({
                commandDefinition: this.definition,
                profileManagerFactory: this.profileFactory
            }).loadProfiles(commandArguments);
            this.log.trace(`Profiles loaded for "${this.definition.name}" command:\n${util_1.inspect(profiles, { depth: null })}`);
            ...
            if (this.definition.profile != null) {
                const profArgs = CliUtils_1.CliUtils.getOptValueFromProfiles(profiles, this.definition.profile, allOpts);
                this.log.trace(`Arguments extract from the profile:\n${util_1.inspect(profArgs)}`);
    ...
    attemptHandlerLoad(response, handlerPath) {
        try {
            ...
            const errorMessage = utilities_1.TextUtils.formatMessage(messages_1.couldNotInstatiateCommandHandler.message, {
    ...
    handleHandlerError(handlerErr, response, handlerPath) {
        ...
        if (handlerErr instanceof error_1.ImperativeError) {
            ...
            if (handlerErr.details.additionalDetails != null
                && typeof handlerErr.details.additionalDetails === "string") {
                response.console.errorHeader("Error Details");
                response.console.error(handlerErr.details.additionalDetails);
            }
            ...
            response.data.setMessage(messages_1.unexpectedCommandError.message + ": " + handlerErr.message);
            this.log.error(`An error was thrown during command execution of "${this.definition.name}". Error Details: ${handlerErr.message}`);
            response.console.errorHeader(messages_1.unexpectedCommandError.message);
 ...
 * @memberof CommandProcessor
 */
CommandProcessor.ERROR_TAG = "Command Processor Error:";
