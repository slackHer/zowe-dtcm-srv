...
const ICommandDefinition_1 = require("../../src/doc/ICommandDefinition");
/**
...
class DefaultHelpGenerator extends AbstractHelpGenerator_1.AbstractHelpGenerator {
    ...
    buildFullCommandHelpText(includeGlobalOptions = true) {
        ...
        if (!this.mProduceMarkdown && !util_1.isNullOrUndefined(this.mCommandDefinition.name) &&
        ...
        if (!util_1.isNullOrUndefined(this.mCommandDefinition.positionals) &&
     ...
     */
    buildUsageDiagram() {
        let usage = this.mRootCommandName + " "
        ...
        if (this.mCommandDefinition.type === "command") {
            ...
            usage = usage.trim();
            if (!util_1.isNullOrUndefined(this.mCommandDefinition.children) && this.mCommandDefinition.children.length > 0) {
    ...
    buildGlobalOptionsSection() {
        ...
        if (!util_1.isNullOrUndefined(this.groupToOption[constants_1.Constants.GLOBAL_GROUP])) {
     ...
     */
    buildHeader(header) {
        return this.renderHelp(util_1.format("{{header}}{{header}}{{header}}{{header}}{{space}}%s\n\n", this.mProduceMarkdown ? header :
    ...
    buildExamplesSection() {
        let examplesText = "";
        if (!util_1.isNullOrUndefined(this.mCommandDefinition.examples)) {
            examplesText = this.mCommandDefinition.examples.map((example) => {
                ...
                let exampleText = utilities_1.TextUtils.wordWrap("{{bullet}}" + exampleHyphen + " {{space}}" + example.description + ":\n\n", undefined, this.mProduceMarkdown ? "" : DefaultHelpGenerator.HELP_INDENT);
     ...
     */
    wordWrapHelp(text) {
        return utilities_1.TextUtils.wordWrap(text, undefined, this.mProduceMarkdown ? "" : DefaultHelpGenerator.HELP_INDENT);
    }
}
/**
