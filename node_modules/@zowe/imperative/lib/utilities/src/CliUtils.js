...
class CliUtils {
    ...
    static getDashFormOfOption(optionName) {
        if (!util_1.isNullOrUndefined(optionName) && optionName.length >= 1) {
     ...
     * @memberof CliUtils
     */
    static getOptValueFromProfiles(profiles, definitions, options) {
        let args = {};
        // Construct the precedence order to iterate through the profiles
        let profileOrder = (definitions.required != null) ? definitions.required : [];
        ...
        profileOrder.forEach((profileType) => {
            ...
            const profile = profiles.get(profileType, false);
