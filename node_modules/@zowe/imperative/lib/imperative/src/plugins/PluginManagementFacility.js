...
const UpdateImpConfig_1 = require("../../src/UpdateImpConfig");
...
class PluginManagementFacility {
    constructor() {
         ...
         * @type {string}
         */
        this.impConfigPropNm = "imperative";
        /**
         ...
         * @type {Object}
         */
        this.npmPkgNmProp = "name";
        this.noPeerDependency = "-1";
        /**
    ...
    init() {
        ...
        PluginRequireProvider.createPluginHooks([
            PMFConstants_1.PMFConstants.instance.IMPERATIVE_PKG_NAME,
            PMFConstants_1.PMFConstants.instance.CLI_CORE_PKG_NAME
        ]);
     ...
     */
    addAllPluginsToHostCli(resolvedCliCmdTree) {
    ...
    loadAllPluginCfgProps() {
        ...
        if (!fs_1.existsSync(this.pmfConst.PLUGIN_JSON)) {
            if (!fs_1.existsSync(this.pmfConst.PMF_ROOT)) {
                this.impLogger.debug("Creating PMF_ROOT directory");
                fs_1.mkdirSync(this.pmfConst.PMF_ROOT);
            }
            this.impLogger.debug("Creating PLUGIN_JSON file");
            jsonfile_1.writeFileSync(this.pmfConst.PLUGIN_JSON, {});
        }
        ...
        for (const nextPluginNm of Object.keys(this.pluginIssues.getInstalledPlugins())) {
            ...
            if (nextPluginCfgProps) {
                ...
                this.impLogger.error("loadAllPluginCfgProps: Unable to load the configuration for the plug-in named '" +
        ...
        for (const [setting, pluginName] of Object.entries(settings_1.AppSettings.instance.getNamespace("overrides"))) {
            if (pluginName !== false) {
                logger_1.Logger.getImperativeLogger().debug(`PluginOverride: Attempting to overwrite "${setting}" with value provided by plugin "${pluginName}"`);
                if (!loadedOverrides.hasOwnProperty(pluginName)) {
                    ...
                    logger_1.Logger.getImperativeLogger().error(overrideErrMsg);
                    ...
                    this.mPluginOverrides[setting] = class {
                        constructor() {
                ...
                let loadedSetting = loadedOverrides[pluginName][setting];
                ...
                if (typeof loadedSetting === "string") {
                    ...
                    try {
                        if (!path_1.isAbsolute(pathToPluginOverride)) {
                            logger_1.Logger.getImperativeLogger().trace(`PluginOverride: Resolving ${pathToPluginOverride} in ${pluginName}`);
                            ...
                            pathToPluginOverride = path_1.join(require.resolve(this.formPluginRuntimePath(pluginName)), "../", pathToPluginOverride);
                        }
                        loadedSetting = require(pathToPluginOverride);
                        logger_1.Logger.getImperativeLogger().info(`PluginOverride: Overrode "${setting}" ` +
                        ...
                        const overrideErrMsg = `Unable to override "${setting}" with "${pathToPluginOverride}" ` +
                            ...
                            `\nWe will use a "${setting}" that purposely fails until you reconfigure.`;
                        PluginIssues_1.PluginIssues.instance.recordIssue(pluginName, PluginIssues_1.IssueSeverity.OVER_ERROR, overrideErrMsg);
                ...
                this.mPluginOverrides[setting] = loadedSetting;
            }
        }
     ...
     */
    requirePluginModuleCallback(relativePath) {
        ...
        try {
            ...
            PluginIssues_1.PluginIssues.instance.recordIssue(this.pluginNmForUseInCallback, PluginIssues_1.IssueSeverity.CMD_ERROR, "Unable to load the following module for plug-in '" +
     ...
     */
    addPluginToHostCli(pluginCfgProps) {
         ...
         */
        let pluginCmdGroup = null;
        try {
            pluginCmdGroup = {
                name: pluginCfgProps.impConfig.name,
                description: pluginCfgProps.impConfig.rootCommandDescription,
                type: "group",
                children: DefinitionTreeResolver_1.DefinitionTreeResolver.combineAllCmdDefs(this.formPluginRuntimePath(pluginCfgProps.pluginName, "./lib"), pluginCfgProps.impConfig.definitions, pluginCfgProps.impConfig.commandModuleGlobs)
            };
            /**
            ...
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, errMsg);
            return;
        }
        ...
        if (pluginCfgProps.impConfig.profiles && pluginCfgProps.impConfig.profiles.length > 0) {
            ...
            try {
                UpdateImpConfig_1.UpdateImpConfig.addProfiles(pluginCfgProps.impConfig.profiles);
            }
            catch (impErr) {
     ...
     *
     */
    comparePluginVersionToCli(pluginName, pluginVerPropNm, pluginVerVal, cliVerPropNm, cliVerVal) {
        ...
        try {
            if (!this.semver.intersects(cliVerVal, pluginVerVal, false)) {
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.WARNING, "The version value (" + pluginVerVal + ") of the plugin's '" +
    ...
    conflictingNameOrAlias(pluginName, pluginGroupDefinition, cmdTreeDef) {
        ...
        if (pluginGroupNm.toLowerCase() === cmdTreeDef.name.toLowerCase()) {
            const conflictMessage = this.impLogger.error("The plugin named '%s' attempted to add a group of commands" +
                " with the name '%s'" +
                ". Your base application already contains a group with the name '%s'.", pluginGroupNm, pluginGroupDefinition.name, cmdTreeDef.name);
        ...
        if (pluginGroupDefinition.aliases != null) {
            for (const pluginAlias of pluginGroupDefinition.aliases) {
                if (pluginAlias.toLowerCase() === cmdTreeDef.name.toLowerCase()) {
                    const conflictMessage = this.impLogger.error("The plugin named '%s' attempted to add a group of commands" +
                        " with the alias '%s' " +
                        ". Your base application already contains a group with the name '%s'.", pluginGroupNm, pluginAlias, cmdTreeDef.name);
                    return { hasConflict: true, message: conflictMessage };
                }
            }
        }
            ...
            for (const nextAliasToTest of cmdTreeDef.aliases) {
                ...
                if (pluginGroupNm.toLowerCase() === nextAliasToTest.toLowerCase()) {
                    const conflictMessage = this.impLogger.error("The plugin attempted to add a group of commands with the name '%s' " +
                        ". Your base application already contains a group with an alias '%s'.", pluginGroupNm, nextAliasToTest, cmdTreeDef.name);
                    return { hasConflict: true, message: conflictMessage };
                }
                if (pluginGroupDefinition.aliases != null) {
                    for (const pluginAlias of pluginGroupDefinition.aliases) {
                        ...
                        if (pluginAlias.toLowerCase() === nextAliasToTest.toLowerCase()) {
                            const conflictMessage = this.impLogger.error("The plugin named '%s' attempted to add a " +
                                ...
                                "another alias of the same name for group '%s'.", pluginGroupDefinition.name, pluginAlias, cmdTreeDef.name);
                            return { hasConflict: true, message: conflictMessage };
                        }
                    }
                }
    ...
    formPluginRuntimePath(pluginName, relativePath = "") {
        const pluginRuntimeDir = path_1.join(this.pmfConst.PLUGIN_NODE_MODULE_LOCATION, pluginName);
        ...
        return path_1.join(pluginRuntimeDir, relativePath);
    }
    // __________________________________________________________________________
    /**
    ...
    loadPluginCfgProps(pluginName) {
        ...
        if (!fs_1.existsSync(pluginRunTimeRootPath)) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The path to the plugin does not exist: " + pluginRunTimeRootPath);
            return null;
        }
        // confirm that we can find the path to the plugin's package.json
        const pluginPkgJsonPathNm = path_1.join(pluginRunTimeRootPath, "package.json");
        if (!fs_1.existsSync(pluginPkgJsonPathNm)) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "Configuration file does not exist: '" + pluginPkgJsonPathNm + "'");
            return null;
        }
        // read package.json
        let pkgJsonData = null;
        try {
            pkgJsonData = jsonfile_1.readFileSync(pluginPkgJsonPathNm);
        }
        catch (ioErr) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "Cannot read '" + pluginPkgJsonPathNm +
        ...
        if (cliPkgName === "NoNameInCliPkgJson") {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.WARNING, "The property '" + this.npmPkgNmProp +
        ...
        if (pkgJsonData.hasOwnProperty(peerDepPropNm)) {
            ...
            if (pkgJsonData[peerDepPropNm].hasOwnProperty(pluginCfgProps.cliDependency.peerDepName)) {
                ...
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.WARNING, "The property '" + pluginCfgProps.cliDependency.peerDepName +
            ...
            if (pkgJsonData[peerDepPropNm].hasOwnProperty(pluginCfgProps.impDependency.peerDepName)) {
                ...
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.WARNING, "The property '" + pluginCfgProps.impDependency.peerDepName +
                    "' does not exist within the '" + peerDepPropNm +
                    "' property in the file '" + pluginPkgJsonPathNm + "'.");
            }
        }
        else {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.WARNING, "Your '" + this.pmfConst.NPM_NAMESPACE +
        ...
        if (!pkgJsonData.hasOwnProperty(this.impConfigPropNm)) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The required property '" + this.impConfigPropNm +
        ...
        try {
            pluginConfig = ConfigurationLoader_1.ConfigurationLoader.load(null, pkgJsonData, this.requirePluginModuleCallback.bind(this));
        }
        catch (impError) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "Failed to load the plugin's configuration from:\n" +
                pluginPkgJsonPathNm +
    ...
    validatePlugin(pluginCfgProps, pluginCmdGroup) {
        if (utilities_1.JsUtils.isObjEmpty(pluginCfgProps.impConfig)) {
            // without a config object, we can do no further validation
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The plugin's configuration is empty.");
            return false;
        }
        ...
        if (!pluginCfgProps.impConfig.hasOwnProperty("name")) {
            ...
            if (pluginCfgProps.npmPackageName === "PluginHasNoNpmPkgName" ||
                ...
                this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The plugin's configuration does not contain an '" +
        ...
        if (pluginCfgProps.impConfig.hasOwnProperty("name")) {
            for (const nextImpCmdDef of this.resolvedCliCmdTree.children) {
                const conflictAndMessage = this.conflictingNameOrAlias(pluginCfgProps.pluginName, pluginCmdGroup, nextImpCmdDef);
                if (conflictAndMessage.hasConflict) {
                    this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, conflictAndMessage.message);
                    break;
                }
            }
        }
        if (!pluginCfgProps.impConfig.hasOwnProperty("rootCommandDescription")) {
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "The plugin's configuration does not contain an '" +
        ...
        if (!pluginCfgProps.impConfig.hasOwnProperty("pluginHealthCheck")) {
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.WARNING, "The plugin's configuration does not contain an '" +
                this.impConfigPropNm + ".pluginHealthCheck' property.");
        }
        else {
            const healthChkModulePath = this.formPluginRuntimePath(pluginCfgProps.pluginName, pluginCfgProps.impConfig.pluginHealthCheck);
            const healthChkFilePath = healthChkModulePath + ".js";
            if (fs_1.existsSync(healthChkFilePath)) {
                ...
                this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The program for the '" + this.impConfigPropNm +
        ...
        if ((!pluginCmdGroup.children || pluginCmdGroup.children.length <= 0) &&
            ...
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The plugin defines no commands and overrides no framework components.");
        }
        else {
         ...
         */
        const pluginConfigToValidate = Object.assign({}, pluginCfgProps.impConfig);
        ...
        try {
            ConfigurationValidator_1.ConfigurationValidator.validate(pluginConfigToValidate);
        }
        catch (impError) {
            this.pluginIssues.recordIssue(pluginCfgProps.pluginName, PluginIssues_1.IssueSeverity.CFG_ERROR, "The plugin configuration is invalid.\nReason = " +
                impError.message);
        }
        return !this.pluginIssues.doesPluginHaveIssueSev(pluginCfgProps.pluginName, [PluginIssues_1.IssueSeverity.CFG_ERROR, PluginIssues_1.IssueSeverity.CMD_ERROR]);
    }
    // __________________________________________________________________________
    /**
     ...
     */
    validatePluginCmdDefs(pluginName, pluginCmdDefs, cmdTreeDepth = 1) {
        for (const pluginCmdDef of pluginCmdDefs) {
            // check for name property
            let pluginCmdName = "NotYetAssigned";
            if (pluginCmdDef.hasOwnProperty("name")) {
                pluginCmdName = pluginCmdDef.name + " (at depth = " + cmdTreeDepth + ")";
            }
            else {
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Command definition at depth " + cmdTreeDepth + " has no 'name' property");
            ...
            if (!pluginCmdDef.hasOwnProperty("description")) {
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Name = '" + pluginCmdName + "' has no 'description' property");
            }
            // check for type property
            if (!pluginCmdDef.hasOwnProperty("type")) {
                this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Name = '" + pluginCmdName + "' has no 'type' property");
            }
            else {
                ...
                if (pluginCmdDef.type.toLowerCase() === "command") {
                    ...
                    if (!pluginCmdDef.hasOwnProperty("handler")) {
                        this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Command name = '" + pluginCmdName + "' has no 'handler' property");
                    }
                    else {
                        // the handler file must exist
                        const handlerModulePath = this.formPluginRuntimePath(pluginName, pluginCmdDef.handler);
                        const handlerFilePath = handlerModulePath + ".js";
                        if (fs_1.existsSync(handlerFilePath)) {
                            // replace relative path with absolute path in the handler property
                            pluginCmdDef.handler = handlerModulePath;
                        }
                        else {
                            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "The handler for command = '" + pluginCmdName +
                    ...
                    if (pluginCmdDef.hasOwnProperty("children")) {
                        if (pluginCmdDef.children.length > 0) {
                            ...
                            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Group name = '" + pluginCmdName +
                                "' has a 'children' property with no children");
                        }
                    }
                    else {
                        // A group must have the children property.
                        this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "Group name = '" + pluginCmdName + "' has no 'children' property");
                    }
                } // end group
            } // end has type
     ...
     */
    validatePluginProfiles(pluginName, pluginProfiles) {
        if (utilities_1.JsUtils.isObjEmpty(pluginProfiles)) {
            this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "The plugin's existing 'profiles' property is empty.");
            return;
        }
        const impHasNoProfiles = utilities_1.JsUtils.isObjEmpty(utilities_1.ImperativeConfig.instance.loadedConfig) ||
            utilities_1.JsUtils.isObjEmpty(utilities_1.ImperativeConfig.instance.loadedConfig.profiles);
        ...
        for (let currProfInx = 0; currProfInx < pluginProfLength; currProfInx++) {
            ...
            while (nextProfInx < pluginProfLength) {
                if (pluginProfiles[currProfInx].type === pluginProfiles[nextProfInx].type) {
                    this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "The plugin's profiles at indexes = '" + currProfInx +
            ...
            for (const impProfile of utilities_1.ImperativeConfig.instance.loadedConfig.profiles) {
                if (pluginProfiles[currProfInx].type === impProfile.type) {
                    this.pluginIssues.recordIssue(pluginName, PluginIssues_1.IssueSeverity.CMD_ERROR, "The plugin's profile type = '" + pluginProfiles[currProfInx].type +
...
//# sourceMappingURL=PluginManagementFacility.js.map